...
export const DAEMON_ENV_SETTINGS: readonly DaemonEnvSetting[] = [
  ...
  { name: 'CCTL_PROBE_UNKNOWN', alias: 'probe-unknown', kind: 'bool' },
  { name: 'CCTL_PROBE_TIMEOUT_MS', alias: 'probe-timeout', kind: 'number' },
  { name: 'CCTL_AUTO_CONTINUE', alias: 'auto-continue', kind: 'bool' },
...
export function resolveDaemonConfig(
  ...
  const probeUnknownEnv = envBool(layered, 'CCTL_PROBE_UNKNOWN');
  const probeUnknown = probeUnknownEnv ?? true;
  const probeTimeoutMs = envNumber(layered, 'CCTL_PROBE_TIMEOUT_MS');
  ...
  const autoContinueEnv = envBool(layered, 'CCTL_AUTO_CONTINUE');
  const autoContinue = autoContinueEnv ?? true;
  const autoContinueMaxAttempts = envNumber(layered, 'CCTL_AUTO_CONTINUE_MAX');
  ...
  const rows: SettingRow[] = [
    ...
    },
    {
      ...
      source: sourceOf('CCTL_PROBE_UNKNOWN', probeUnknownEnv !== undefined),
      detail:
    ...
    },
    {
      ...
      source: sourceOf('CCTL_PROBE_TIMEOUT_MS', probeTimeoutMs !== undefined),
    ...
    },
    {
      ...
      source: sourceOf('CCTL_AUTO_CONTINUE', autoContinueEnv !== undefined),
      detail:
    ...
    },
    {
      ...
      source: sourceOf('CCTL_AUTO_CONTINUE_MAX', autoContinueMaxAttempts !== undefined),
    ...
    },
    {
      ...
      value: logFilePath,
      source: sourceOf('CCTL_LOG_FILE', logFileEnv !== undefined),
      detail:
