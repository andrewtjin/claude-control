...
Registers (or updates, or confirms unchanged) this host's autostart — a Windows logon
Scheduled Task, the same task registered from inside WSL, a macOS LaunchAgent, or a
Linux systemd user unit (see `docs/PLATFORM.md`) — then tries to start the daemon
immediately rather than making you wait for the next logon. A failure to start now is
reported but does not undo the registration:
...
On a host with no autostart backend (a Linux box with neither Windows interop nor a
systemd user manager, or `CCTL_AUTOSTART_BACKEND=none` — see `docs/PLATFORM.md`) the
wizard registers nothing and says so, then checks the heartbeat once instead of
waiting for a daemon it did not start:

```
Autostart is not available on this platform — run the daemon yourself: cctl daemon supervise (...)
